...
import { loadOrCreateDestinationKeypair } from "../../email-relay/keypair.js";

describe("loadOrCreateDestinationKeypair (email-relay/keypair.ts)", () => {
  let dir: string;

  beforeEach(() => {
    dir = mkdtempSync(path.join(tmpdir(), "arald-email-relay-keypair-"));
  });
  ...
  it("creates parent directories that don't exist yet", () => {
    const file = path.join(dir, "nested", "keys", "hq.json");
