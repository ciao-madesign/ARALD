...
import { LoraMedium, LoraSimulatedTransport } from "../../node/src/transports/lora.js";
...
/**
 * Spec §67, docs/next-steps.md Opzione L: a simulated LoRa `Transport`, no
 * real radio, affiancato a `BleSimulatedTransport` (not a replacement — see
 * `tests/integration/mixed-transport.test.ts`). Built on the same
 * `SimulatedLinkTransport` base as BLE (`node/src/transports/simulated-link.ts`)
 * — this file exercises the exact same scenario set as
 * `tests/unit/ble-transport.test.ts`, at LoRa's own MTU/latency defaults,
 * to prove the shared base behaves correctly under a second configuration,
 * not just the one it was originally extracted from.
 */
describe("LoraSimulatedTransport", () => {
  const transports: LoraSimulatedTransport[] = [];
  function makeTransport(
    deviceId: string,
    options?: ConstructorParameters<typeof LoraSimulatedTransport>[2],
  ): { transport: LoraSimulatedTransport; nodeId: string } {
    const nodeId = Identity.generate().nodeId;
    const t = new LoraSimulatedTransport(nodeId, deviceId, options);
    transports.push(t);
  ...
  it("connects, exchanges HELLO, and each side learns the other's node id", async () => {
    const a = makeTransport("device-a", { latencyMs: 0 });
    const b = makeTransport("device-b", { latencyMs: 0 });
  ...
  it("reassembles fragments correctly even when they arrive out of order", async () => {
    const a = makeTransport("device-a", { mtu: 200, latencyMs: 1 });
    const b = makeTransport("device-b", { mtu: 200, latencyMs: 1 });
    await Promise.all([a.transport.start(), b.transport.start()]);
    const peerId = await a.transport.connect({ host: "device-b", port: 0 });
    ...
    const payloads = [{ n: 1, text: "a".repeat(3000) }, { n: 2, text: "b".repeat(3000) }, { n: 3, text: "c".repeat(3000) }];
    await Promise.all(
  ...
  it("rejects a connection attempt once the target is at its own connection limit", async () => {
    const target = makeTransport("device-target", { maxConnections: 1, latencyMs: 0 });
    const first = makeTransport("device-1", { latencyMs: 0 });
    const second = makeTransport("device-2", { latencyMs: 0 });
  ...
  it("two separate LoraMedium instances cannot see or reach each other, simulating out-of-range meshes", async () => {
    const medium1 = new LoraMedium();
    const medium2 = new LoraMedium();
    const a = makeTransport("device-a", { medium: medium1, latencyMs: 0 });
    const b = makeTransport("device-b", { medium: medium2, latencyMs: 0 });
    await Promise.all([a.transport.start(), b.transport.start()]);

    await expect(a.transport.connect({ host: "device-b", port: 0 })).rejects.toThrow(/no LoRa device advertising/);
  });
  ...
  it("connect() rejects when the target device isn't advertising at all", async () => {
    const a = makeTransport("device-a", { latencyMs: 0 });
    await a.transport.start();
    await expect(a.transport.connect({ host: "device-nowhere", port: 0 })).rejects.toThrow(/no LoRa device advertising/);
  });

  it("uses a much higher default latency than BLE, distinguishing the two radios by speed rather than MTU", async () => {
    // Regression-by-design: confirms DEFAULT_LATENCY_MS is really wired through, not silently
    // ignored — connect() (one HELLO fragment at the default 200-byte MTU) takes noticeably longer
    // than an instant/near-zero-latency link would.
    ...
    const start = Date.now();
    await a.transport.connect({ host: "device-b", port: 0 });
    expect(Date.now() - start).toBeGreaterThanOrEqual(200); // DEFAULT_LATENCY_MS is 250ms; generous margin below it
  });
});
