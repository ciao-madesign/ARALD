...
- [`docs/beacon.md`](docs/beacon.md) — NOMAD Card: dispositivo radio unico (formato carta di credito) che unifica Beacon ed Emergency Relay in tre profili firmware (Beacon/Relay/Beacon+Relay Mode), più Fixed Relay/Relay Registry e una nota sulla conformità normativa UE (RED, ETSI EN 300 328/300 220, CE) per un'eventuale commercializzazione futura — il **dispositivo fisico** resta a livello di proposta (nessun hardware costruito), ma la **logica di rete** dietro tutti e tre i profili è implementata e testata (`docs/security.md` voci #54-56: Relay Registry, profilo corriere, Beacon SOS)
...
- [`docs/test-protocol.md`](docs/test-protocol.md) — protocollo di test/validazione tecnica a fasi (0-8, progressione Box/Portable/Card/Smartphone, test T-number, KPI, criteri PASS/CONDITIONAL/FAIL) — asse complementare a `emergency-rescue-network.md` (qui la scala *tecnica*, lì budget/partner), documentazione di riferimento, nessun codice nuovo scritto per questo documento (a parte un prerequisito già completato, `docs/security.md` voce #58)
- [`docs/emergency-portal.md`](docs/emergency-portal.md) — proposta di architettura per un **Emergency Portal** web (Portal/Backend/Network a tre livelli, multi-tenant per organizzazione, "ARALD Box" come gateway Internet↔mesh) per enti/operatori (CNSAS, Protezione Civile, gestori di rifugio) — documentazione di riferimento, nessun codice; valuta la proposta contro i meccanismi mesh già esistenti (Relay Registry, Emergency Beacons, Drops, Node Append) e segnala esplicitamente una duplicazione di terminologia non ancora risolta ("ARALD" nella proposta contro "Nomad-Net"/"NOMAD" nel resto del repository)
...
**Cosa resta bloccato**: le versioni **hardware/Docker reali** di BLE e del gateway NOMAD (serve rispettivamente hardware BLE fisico e Docker + un'istanza Project NOMAD raggiungibile) — le loro controparti simulate/mockate sopra hanno comunque validato tutta la logica applicativa in puro software. **Espansioni proposte, hardware mai costruito**: NOMAD-NET BOX/PORTABLE (`docs/deployment.md`, nessun codice) e la NOMAD Card (`docs/beacon.md`, dispositivo radio unico che unifica Beacon Mode/Relay Mode/Beacon+Relay Mode in un'unica architettura hardware, più Fixed Relay/Relay Registry) — per quest'ultima, a differenza di BOX/PORTABLE, la **logica di rete è già implementata e testata** in puro software (`node/src/relay-registry.ts`, `emergency-beacon.ts`, `transports/beacon-broadcast.ts`, modifiche a `store-and-forward.ts` — `docs/security.md` voci #54-56): resta bloccato solo il dispositivo fisico stesso (PCB, firmware, enclosure), non disponibile in questo ambiente. **Debito di design della UI mobile: affrontato** — una passata di rebrand completa ("Waypoint", palette topografica, marchio, componenti) più una passata UX dedicata per un pubblico generico e non tecnico sono già state realizzate (vedi `docs/security.md`, voci #28-31 e #48); resta annotato solo un possibile restyling grafico futuro per un aspetto più moderno, non ancora pianificato in dettaglio. Vedi [`docs/roadmap.md`](docs/roadmap.md) per lo stato dettagliato di ogni milestone e [`docs/next-steps.md`](docs/next-steps.md) per i piani d'azione.
