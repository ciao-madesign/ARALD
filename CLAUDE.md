...
Lo stato ad oggi (23 agosto 2026): non ci sono candidati aperti realizzabili in puro software rimasti "in sospeso a metà" — ogni pezzo iniziato è stato portato a termine col workflow completo. Il debito di design della UI mobile è stato affrontato (#28-31, incluso il rebrand "Waypoint"); `ContentStore` è stato limitato per dimensione (#32); i primi due pezzi dell'evoluzione news (parser RSS/Atom + schema arricchito, livelli headline/summary) sono fatti (#33, #35); il meccanismo di priorità/broadcast per contenuto è fatto (#34). Restano invece diverse **valutazioni** (non implementazioni, salvo i pezzi già fatti) di proposte dell'utente, tutte documentate in dettaglio in `docs/next-steps.md`: il resto dell'evoluzione di `service://news` (digest AI, `service://emergency-news` — Opzione I), e integrare sotto-applicazioni di messaggistica stile BitChat (chat 1:1, canali pubblici, chat private/di gruppo cifrate) e tracciamento posizione opportunistico (Opzione J). `docs/next-steps.md`, sezione "Piano d'insieme", riordina tutti i candidati rimasti in un'unica sequenza motivata da dipendenze reali — vedi sotto per il riassunto aggiornato. Le uniche milestone ancora aperte nella loro forma *reale* (8/9/10/11/14/17/18/19, vedi `docs/roadmap.md`) restano bloccate su prerequisiti esterni (hardware fisico, Docker + Project NOMAD raggiungibile, accesso a internet reale) non disponibili in questo ambiente.

## Priorità per chi riprende questo lavoro (handoff)

`docs/next-steps.md`, sezione "Piano d'insieme", è la fonte di verità per l'ordine — **leggila per intero prima di iniziare qualunque lavoro nuovo**. Le prime tre fasi originarie (debito di design UI mobile, `ContentStore` senza limite, meccanismo di priorità/broadcast per contenuto) sono state completate (#28-34, `docs/security.md`), così come i primi due pezzi della Fase 3 (parser RSS #33, livelli headline/summary #35); restano le fasi successive:

1. **Feature veloci e in gran parte parallelizzabili**: chat 1:1 (backend già pronto — `sendPrivateMessage()` è già E2E cifrato, multi-hop, delay-tolerant; serve solo UI, ora sopra il nuovo design "Waypoint") e l'ultimo pezzo backend dell'evoluzione news ancora da fare (digest AI componendo `NewsGateway`+`AiGateway` — parser RSS, livelli headline/summary e meccanismo di broadcast sono già fatti, #33/#35/#34).
