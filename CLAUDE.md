...
Lo stato ad oggi (22 agosto 2026): non ci sono candidati aperti realizzabili in puro software rimasti "in sospeso a metà" — ogni pezzo iniziato è stato portato a termine col workflow completo. Il debito di design della UI mobile è stato affrontato (#28-31, incluso il rebrand "Waypoint"); `ContentStore` è stato limitato per dimensione (#32); il primo pezzo dell'evoluzione news (parser RSS/Atom + schema arricchito) è fatto (#33). Restano invece diverse **valutazioni** (non implementazioni, salvo il pezzo 1 di news già fatto) di proposte dell'utente, tutte documentate in dettaglio in `docs/next-steps.md`: il resto dell'evoluzione di `service://news` (priorità di emergenza, digest AI — Opzione I), e integrare sotto-applicazioni di messaggistica stile BitChat (chat 1:1, canali pubblici, chat private/di gruppo cifrate) e tracciamento posizione opportunistico (Opzione J). `docs/next-steps.md`, sezione "Piano d'insieme", riordina tutti i candidati rimasti in un'unica sequenza motivata da dipendenze reali — vedi sotto per il riassunto aggiornato. Le uniche milestone ancora aperte nella loro forma *reale* (8/9/10/11/14/17/18/19, vedi `docs/roadmap.md`) restano bloccate su prerequisiti esterni (hardware fisico, Docker + Project NOMAD raggiungibile, accesso a internet reale) non disponibili in questo ambiente.

## Priorità per chi riprende questo lavoro (handoff)

`docs/next-steps.md`, sezione "Piano d'insieme", è la fonte di verità per l'ordine — **leggila per intero prima di iniziare qualunque lavoro nuovo**. Le prime due fasi originarie (debito di design UI mobile, `ContentStore` senza limite) sono state completate (#28-32, `docs/security.md`), e il primo pezzo della Fase 3 (parser RSS) è fatto (#33); restano le fasi successive:

1. **Il prerequisito architetturale condiviso**: un meccanismo di priorità/broadcast per singolo contenuto (oggi assente — la priorità è fissa per tipo di pacchetto, la scoperta contenuti è pull-based) serve sia a `service://emergency-news` (Opzione I, news) sia ai canali pubblici di chat (Opzione J, messaggistica) — un solo pezzo di lavoro, due casi d'uso, da costruire una volta sola qui. L'utente ha già indicato la direzione: un nuovo pacchetto di annuncio broadcast firmato, parallelo a `SERVICE_ANNOUNCE`, non un campo di priorità dichiarata propagato nel sync esistente.
2. **Feature veloci e in gran parte parallelizzabili, restanti**: chat 1:1 (backend già pronto — `sendPrivateMessage()` è già E2E cifrato, multi-hop, delay-tolerant; serve solo UI, ora sopra il nuovo design "Waypoint") e i pezzi backend dell'evoluzione news ancora da fare (livelli headline/summary, digest AI componendo `NewsGateway`+`AiGateway` — il parser RSS/schema arricchito è già fatto, #33).
3. **Canali pubblici di chat + `service://emergency-news`**, costruiti sopra il prerequisito del punto 1.
4. **Tracciamento posizione** — richiede prima una discussione esplicita con l'utente su consenso/conservazione/autorizzazione (la specifica stessa, §56, elenca la posizione tra i dati da minimizzare) prima di scrivere il gateway registro o il lato mobile. **Chat private/di gruppo cifrate** — l'ultimo, perché richiede il design crittografico nuovo (key-wrapping per una chiave di gruppo, oggi esiste solo cifratura punto-a-punto) più sostanzioso di tutti gli altri candidati.
