...
Lo stato ad oggi (22 agosto 2026): non ci sono candidati aperti realizzabili in puro software rimasti "in sospeso a metà" — ogni pezzo iniziato è stato portato a termine col workflow completo. Il primo dei tre punti aperti originali (debito di design della UI mobile) è stato affrontato nella voce #28; il terzo (`ContentStore` senza limite di dimensione) nella voce #32. Il secondo, la proposta "Nomad News evoluto", è in corso: valutata in dettaglio (`docs/next-steps.md` Opzione I) e ora in implementazione un pezzo alla volta con ok esplicito dell'utente tra un pezzo e il successivo — il pezzo 1 di 4 (parser RSS/Atom + schema arricchito) è fatto (voce #33), restano i pezzi 2-4. Le uniche milestone ancora aperte nella loro forma *reale* (8/9/10/11/14/17/18/19, vedi `docs/roadmap.md`) restano bloccate su prerequisiti esterni (hardware fisico, Docker + Project NOMAD raggiungibile, accesso a internet reale) non disponibili in questo ambiente.

## Priorità per chi riprende questo lavoro (handoff)

Un solo filone aperto rimasto (gli altri due punti originali, debito di design della UI mobile e limite di dimensione su `ContentStore`, sono stati affrontati rispettivamente nelle voci #28 e #32 di `docs/security.md`):

1. **Proposta "Nomad News evoluto", in corso**: 4 pezzi pianificati in `docs/next-steps.md` Opzione I, da fare uno alla volta con ok esplicito tra un pezzo e il successivo (scelta dell'utente). Pezzo 1 (parser RSS/Atom + schema arricchito) fatto — voce #33 di `docs/security.md`. Restano: (2) livelli headline/summary/articolo completo, (3) digest AI componendo `NewsGateway`+`AiGateway`, (4) il meccanismo di priorità/broadcast per contenuto (il pezzo architetturale genuinamente nuovo — oggi la priorità è fissa per tipo di pacchetto; l'utente ha già scelto la direzione: un nuovo pacchetto di annuncio broadcast firmato parallelo a `SERVICE_ANNOUNCE`, non un campo di priorità dichiarata propagato nel sync esistente) → `service://emergency-news` come conseguenza naturale del pezzo 4.

Non è urgente/bloccante — resta un filone aperto per la prossima sessione, non un bug che rompe qualcosa oggi. Vedi anche la nota sul futuro restyling grafico della UI mobile più sopra ("Stato del progetto") — un altro punto annotato ma non ancora pianificato in dettaglio.
