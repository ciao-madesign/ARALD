...
Lo stato ad oggi (24 agosto 2026): non ci sono candidati aperti realizzabili in puro software rimasti "in sospeso a metà" — ogni pezzo iniziato è stato portato a termine col workflow completo. Il debito di design della UI mobile è stato affrontato (#28-31, incluso il rebrand "Waypoint"); `ContentStore` è stato limitato per dimensione (#32); i primi due pezzi dell'evoluzione news (parser RSS/Atom + schema arricchito, livelli headline/summary) sono fatti (#33, #35); il meccanismo di priorità/broadcast per contenuto è fatto (#34); la chat 1:1 è fatta (#36). Restano invece diverse **valutazioni** (non implementazioni, salvo i pezzi già fatti) di proposte dell'utente, tutte documentate in dettaglio in `docs/next-steps.md`: il resto dell'evoluzione di `service://news` (digest AI, `service://emergency-news` — Opzione I), e integrare sotto-applicazioni di messaggistica stile BitChat (canali pubblici, chat private/di gruppo cifrate — la chat 1:1 è già fatta) e tracciamento posizione opportunistico (Opzione J). `docs/next-steps.md`, sezione "Piano d'insieme", riordina tutti i candidati rimasti in un'unica sequenza motivata da dipendenze reali — vedi sotto per il riassunto aggiornato. Le uniche milestone ancora aperte nella loro forma *reale* (8/9/10/11/14/17/18/19, vedi `docs/roadmap.md`) restano bloccate su prerequisiti esterni (hardware fisico, Docker + Project NOMAD raggiungibile, accesso a internet reale) non disponibili in questo ambiente.

## Priorità per chi riprende questo lavoro (handoff)

`docs/next-steps.md`, sezione "Piano d'insieme", è la fonte di verità per l'ordine — **leggila per intero prima di iniziare qualunque lavoro nuovo**. Le prime tre fasi originarie (debito di design UI mobile, `ContentStore` senza limite, meccanismo di priorità/broadcast per contenuto) sono state completate (#28-34, `docs/security.md`), così come i primi tre pezzi della Fase 3 (chat 1:1 #36, parser RSS #33, livelli headline/summary #35); restano le fasi successive:

1. **Ultimo pezzo veloce rimasto**: il digest AI componendo `NewsGateway`+`AiGateway` (parser RSS, livelli headline/summary, meccanismo di broadcast e chat 1:1 sono già fatti, #33/#35/#34/#36).
2. **Canali pubblici di chat + `service://emergency-news`**, costruiti sopra `CONTENT_ANNOUNCE` (#34) — pronto da usare.
3. **Tracciamento posizione** — richiede prima una discussione esplicita con l'utente su consenso/conservazione/autorizzazione (la specifica stessa, §56, elenca la posizione tra i dati da minimizzare) prima di scrivere il gateway registro o il lato mobile. **Chat private/di gruppo cifrate** — l'ultimo, perché richiede il design crittografico nuovo (key-wrapping per una chiave di gruppo, oggi esiste solo cifratura punto-a-punto) più sostanzioso di tutti gli altri candidati.
...
- `message-history.ts` — `MessageHistory` (voce #36), cronologia locale (mai firmata/propagata) dei messaggi 1:1 inviati/ricevuti via `sendPrivateMessage()`/`handlePrivateMessage()`, bounded su conversazioni (`maxPeers`, eviction pesata sulla fiducia) e messaggi-per-conversazione (`maxMessagesPerPeer`). Esporta anche `MAX_MESSAGE_TEXT_LENGTH`, il limite canonico di lunghezza di un singolo messaggio — importato sia da `node.ts` (`extractChatText()`, applicato sia in invio che in ricezione) sia da `web-ui.ts` (`POST /api/messages`), un solo posto per non farli divergere.
...
- `web-ui.ts` — interfaccia web locale di stato/ricerca (spec §59), server `node:http` minimale. Endpoint di sola lettura: `/api/status` (include `relaying`, da `NomadNode.canRelayNow()`), `/api/peers` (`canMessage` per peer, voce #36), `/api/services`, `/api/content` (tutto il conosciuto), `/api/search?q=...` (filtrato), `/api/pairing` (nome rete/password/indirizzo/QR di pairing, non autenticato). Più `POST /api/call` e `GET`/`POST /api/messages` (endpoint dietro password di rete via un `isAuthorized()` condiviso — scrittura disattivata di default; `/api/messages` è l'unica coppia di endpoint che richiede auth anche in lettura, perché il testo dei messaggi è "Private messages" spec §56, non stato pubblico della mesh — base server-side per l'app mobile, `docs/next-steps.md` Opzione H, vedi `docs/security.md` voci #22/#24/#25/#36).
