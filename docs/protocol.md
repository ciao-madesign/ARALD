...
**Limite noto, rilevante per direzioni future (es. una proposta di "notizie di emergenza" e una di canali di chat pubblici, entrambe valutate in `docs/next-steps.md`)**: oggi la priorità è fissa **per tipo di messaggio**, mai parametrizzabile per singolo contenuto — ogni pacchetto `CONTENT_*` usa sempre `Priority.CONTENT`, indipendentemente da cosa contiene. Inoltre la scoperta dei contenuti è **pull-based** (`CONTENT_QUERY`/`CONTENT_FOUND`): non esiste un annuncio broadcast per contenuti, a differenza dei servizi (`SERVICE_ANNOUNCE`). Un contenuto che debba davvero "saltare la coda e raggiungere subito tutta la mesh" (es. un bollettino di emergenza, o un nuovo messaggio in un canale di chat pubblico che dovrebbe "spingere" a chi è già connesso invece di aspettare una query esplicita) non è quindi gratis con l'architettura attuale — richiederebbe o un nuovo meccanismo di annuncio broadcast per contenuti ad alta priorità (parallelo a `SERVICE_ANNOUNCE`, stessa disciplina di firma), oppure una priorità dichiarata propagata attraverso il catalog sync esistente (`SYNC_RESPONSE`). Non implementato — un solo pezzo di lavoro che servirebbe entrambi i casi d'uso, isolato come prerequisito condiviso in `docs/next-steps.md`, sezione "Piano d'insieme".
