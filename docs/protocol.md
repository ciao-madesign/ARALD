...
| `GROUP_MESSAGE` | messaggio di una chat di gruppo cifrata end-to-end (spec §56) — broadcast (nessun `destination`), payload cifrato con la chiave simmetrica del gruppo (AES-256-GCM) più una firma Ed25519 del mittente; l'invito che distribuisce la chiave viaggia invece come `PRIVATE_MESSAGE` 1:1, nessun pacchetto dedicato | Milestone 15 (seguito audit, `docs/security.md` voce #42) |
...
Ogni pacchetto `CONTENT_*` legato al ciclo pull (`CONTENT_QUERY`/`CONTENT_FOUND`/...) usa sempre `Priority.CONTENT`, indipendentemente da cosa contiene — la priorità resta fissa per tipo di messaggio per quella famiglia. Un contenuto che debba "saltare la coda e raggiungere subito tutta la mesh" (es. un bollettino di emergenza, o un nuovo messaggio in un canale di chat pubblico) usa invece `CONTENT_ANNOUNCE` (vedi sopra, `docs/security.md` voce #34): `publishContent(..., { announce: true, priority })` origina un annuncio broadcast alla priorità dichiarata, parallelo a `SERVICE_ANNOUNCE`. **Limite noto, accettato**: quella priorità non è autenticata più di quanto lo sia `packet.source` (`CLAUDE.md`) — un peer può già dichiarare qualunque priorità su qualunque pacchetto che origina; il rate limiting per-peer esistente (applicato prima dello switch sui tipi in `handlePacket()`) resta la difesa principale contro un flood a priorità `EMERGENCY` **ricevuto da un peer**. Il primo consumatore reale di questo meccanismo è `service://emergency-news` (`docs/security.md` voce #39, `gateway/nomad/news-gateway.ts`) — e ha esposto la metà mancante del problema: quel rate limiting non copre un pacchetto che *questo stesso nodo* origina da sé (`floodExcept()` non passa mai per `handlePacket()`), quindi un `NewsGateway` alimentato da un feed `--news-url` ostile poteva flooder la mesh a priorità `EMERGENCY` senza alcun limite. `NewsGateway` applica ora il proprio tetto locale (`MAX_EMERGENCY_ANNOUNCES_PER_SYNC`) — una mitigazione specifica al chiamante, non un cambiamento al meccanismo `CONTENT_ANNOUNCE` stesso, che resta generico e senza un limite di questo tipo incorporato: un futuro chiamante che origina i propri `CONTENT_ANNOUNCE` a priorità alta dovrà applicare una protezione analoga per conto proprio.

I canali pubblici di chat (`docs/security.md` voce #40, `node/src/public-channels.ts`) sono arrivati subito dopo e hanno esposto la stessa lacuna su un pacchetto diverso: `considerChannelMessage()` (node.ts) chiama `getContent()` — che flooda un `CONTENT_QUERY`, non un `CONTENT_ANNOUNCE` — per ogni entry accettata con nome `chat:<canale>`; un burst di entry fasulle poteva far originare al nodo un `CONTENT_QUERY` per ognuna, di nuovo senza alcun limite (il rate limiting per-peer copre solo ciò che il nodo *riceve*, mai ciò che *origina*). Stessa mitigazione locale al chiamante, non al meccanismo generico: un nuovo `MAX_CONCURRENT_CHANNEL_FETCHES` limita quanti fetch reattivi `considerChannelMessage()` tiene in volo contemporaneamente. Il pattern che emerge da entrambi i casi: **qualunque codice che origina un pacchetto (di qualunque tipo) in risposta a input di rete deve applicare il proprio limite locale** — `handlePacket()`'s rate limiting non copre e non può coprire questo lato per costruzione. `GROUP_MESSAGE` (voce #42) non ricade in questo pattern: è sempre originato esplicitamente via l'endpoint HTTP autenticato `POST /api/group-messages`, mai automaticamente in risposta a traffico di rete in arrivo, quindi non c'è una catena di amplificazione self-originated da proteggere per questo tipo.

`GROUP_MESSAGE` espone invece una lacuna diversa, non di rate ma di **dedup**: `SeenCache` (routing.ts) deduplica un pacchetto broadcast solo per il suo `packet.id` casuale, mai per il contenuto del payload — per `CONTENT_ANNOUNCE`/`SERVICE_ANNOUNCE`/canali pubblici questo non è mai stato un problema pratico perché il contenuto stesso è identificato da un content id stabile (`computeContentId()`) e la struttura che lo riceve (`ContentStore`/`RemoteCatalog`/`PublicChannels`) dedupe per quello a valle. `GROUP_MESSAGE` non aveva un equivalente: un payload catturato e re-incapsulato in un pacchetto con un `packet.id` nuovo bypassava `SeenCache` e veniva ri-accettato indefinitamente da chi era membro del gruppo. Corretto (`docs/security.md` voce #42) con `GroupMessage.messageId` — la firma Ed25519 del mittente stessa, deterministica per EdDSA (stessa chiave + stessi byte firmati → sempre la stessa firma) — usata da `Groups.recordMessage()` per il dedup, stesso principio "identificatore stabile del contenuto, non del pacchetto" degli altri meccanismi broadcast. **Limite noto, non risolto qui**: questo fix copre solo la duplicazione lato memorizzazione — il replay continua comunque a essere re-floodato dalla mesh (la decisione di forward avviene in `decideForward()` prima che l'handler del tipo veda mai il pacchetto), esattamente come per `CONTENT_ANNOUNCE`/`SERVICE_ANNOUNCE`/canali pubblici, nessuno dei quali ha una dedup di forwarding basata sul contenuto — un limite architetturale condiviso da ogni pacchetto broadcast di questo prototipo, non specifico a `GROUP_MESSAGE`.
