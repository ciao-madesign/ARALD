...
| 9 | Smartphone (app mobile minimale) | ✅ Fase 1 completata funzionalmente (verificata via browser), UI da riprogettare — build nativa reale ⏳ bloccata | `mobile/`, client Capacitor verso un gateway (`docs/next-steps.md` Opzione H); Fase 2 (Bluetooth) bloccata sullo stesso hardware dell'Opzione A, la build Android nativa su `dl.google.com` non raggiungibile da questa sessione. **Design debt esplicito**: mai una vera passata di design, solo iterazioni funzionali — vedi `docs/next-steps.md`, sezione "Debito di design della UI mobile" |
...
| 11 | Gateway (`gateway/nomad/`) | ✅ Mockato (seguito audit, Slice 9-10 + voce #27) — Docker reale ⏳ bloccato | `kiwix-gateway.ts` (`content://`, `service://kiwix-search`) + `ai-gateway.ts` (`service://ai`) + `news-gateway.ts` (`service://news`), verificati contro `FakeNomadServer`/`FakeOllamaServer`/un server di test locale. Proposta di evoluzione del gateway news (RSS reale, priorità di emergenza, digest AI) valutata ma non implementata — vedi `docs/next-steps.md` Opzione I |
