...
Seguendo il principio bottom-up (§89), il prossimo passo naturale dopo questo commit è la Milestone 8 (BLE transport) **oppure**, se si vuole prima consolidare il livello servizi, la Milestone 10-11 (NOMAD + gateway) — entrambe possono procedere in parallelo perché non condividono codice con il routing già validato. Non è raccomandato iniziare la Milestone 15 (sicurezza) prima di aver validato almeno un transport reale oltre a TCP, per evitare di dover ridisegnare il modello di trust attorno a un solo transport.
