...
| 9 | Smartphone (app mobile minimale) | ✅ Fase 1 completata (verificata via browser) — build nativa reale ⏳ bloccata | `mobile/`, client Capacitor verso un gateway (`docs/next-steps.md` Opzione H); Fase 2 (Bluetooth) bloccata sullo stesso hardware dell'Opzione A, la build Android nativa su `dl.google.com` non raggiungibile da questa sessione |
