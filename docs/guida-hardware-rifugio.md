# Guida hardware — installare un nodo ARALD in un rifugio
...
|---|---|---|---|
| **GL.iNet GL-MT3000 "Beryl AX"** | MediaTek MT7981 (ARM Cortex-A53), 512 MB RAM, 256 MB flash, porta USB 3.0 per spazio extra | 50-65 € | **Scelta di riferimento per questo progetto** — RAM/flash abbondanti per far girare ARALD senza problemi, consumo bassissimo (2,6-5 W, ottimo con pannello solare), formato tascabile |
...
|---|---|---|---|
| **Router Wi-Fi con OpenWrt preinstallato** (uno dei modelli consigliati sopra) | il "cervello" del nodo: fa girare il software ARALD **e** crea già di suo la rete Wi-Fi a cui si collegano gli ospiti — nessun passo di configurazione separato per il punto di accesso | vedi tabella sopra | Va scelto un modello con **almeno 256 MB di RAM e una porta USB** per lo spazio extra (il software ARALD più le sue dipendenze non entrano comodamente nella sola memoria flash interna dei modelli più economici) |
| **Chiavetta USB, almeno 8 GB** (solo se il router scelto ha poca memoria flash interna) | spazio extra dove installare il software ARALD e le sue dipendenze, oltre al piccolo spazio di fabbrica del router | 5-10 € | Non serve se il router ha già memoria interna abbondante (es. eMMC) — verificare le specifiche del modello scelto |
...
Il dispositivo scelto sopra (router o scheda singola economica) **basta da solo** per bacheca, mappe, messaggi e contenuti già pronti — non serve nient'altro per far funzionare un nodo ARALD. Un'intelligenza artificiale locale (per rispondere a domande tipo "qual è il sentiero più semplice per il bivacco X?") è invece un pezzo **in più**, opzionale, che richiede un secondo dispositivo più potente — pensalo come "un cameriere piccolo e sempre presente" (il dispositivo di sopra) più "una cucina sul retro" (questo secondo dispositivo) che entra in gioco solo per le domande all'AI.
...
**Decisione architetturale (31 agosto 2026, vedi `docs/deployment.md` "L'ARALD Hub come sistema portatile")**: questo secondo dispositivo non deve più intendersi come un computer specifico da comprare e tenere fisso — l'intero ambiente software (Project NOMAD: sistema operativo, Docker, dati, modelli AI) può vivere su un **SSD USB esterno avviabile**, spostabile su qualunque PC/mini-PC compatibile (architettura x86-64, boot da USB, RAM sufficiente) disponibile al momento — l'esempio concreto: un SSD Netac da 250 GB, avviato oggi su un PC Acer come piattaforma di sviluppo, spostabile su un host migliore in futuro senza rifare nulla. Non cambia nulla di quanto scritto sopra su dove va tenuto/cosa cambia per l'ospite; cambia solo che "comprare il secondo dispositivo" diventa opzionale se un PC compatibile è già disponibile — basta l'SSD.
...
**Terza opzione (2 settembre 2026, vedi `docs/deployment.md` "ARALD Box e PORTABLE")**: oltre a un mini-PC usato e a un SSD avviabile (ARALD Portable, sopra), esiste ora anche una configurazione di riferimento per un dispositivo dedicato singolo — **ARALD Box**: una scheda Orange Pi 4 Pro (4 GB) più un SSD NVMe interno da 256 GB (~20-30 € per l'SSD, budget indicativo per il modello di riferimento — non serve un SSD veloce/premium), dissipatore e alimentatore USB-C. A differenza delle prime due opzioni non richiede un computer separato da trovare/riusare, ma è anche l'unica delle tre a richiedere l'acquisto di hardware dedicato invece di riusare qualcosa già disponibile. Resta comunque **non ancora verificata con un benchmark reale** in questo progetto (stessa nota in cima a questa guida) — il chip indicato (Allwinner A733) è riportato come fornito dalla specifica ricevuta, non verificato in modo indipendente.

**Preparazione**: fuori dallo scopo di questa guida per ora (richiede installare Ollama e un modello di linguaggio leggero) — chi vuole aggiungere questo pezzo dovrebbe chiedere una mano a chi segue il progetto ARALD. Per il confronto tecnico completo tra le diverse opzioni hardware per questo secondo dispositivo, vedi `docs/deployment.md`.
...
2. **Rinominare la rete Wi-Fi** (SSID) e impostare una **password** breve e facile da leggere ad alta voce (lo stesso stile già usato dall'app mobile del progetto — vedi `mobile/README.md` e `docs/security.md` voce #24), es. nome rete `ARALD-<nome del rifugio>`.
...
5. **Copiare il software ARALD** sul router (la cartella `node/` di questo repository, già compilata — `npm run build -w node`) e configurarlo per **avviarsi automaticamente all'accensione** — su OpenWrt, uno script di avvio in `/etc/init.d/` gestito da `procd` (il sistema di avvio automatico dei servizi), non un comando da digitare ogni volta.
...
3. Nelle opzioni avanzate di Raspberry Pi Imager, **configurare la rete Wi-Fi del dispositivo come punto di accesso** (access point) con:
   - un **nome rete** riconoscibile, ad esempio `ARALD-<nome del rifugio>` (es. `ARALD-RIFUGIO-ALPI`);
...
4. **Copiare il software ARALD** sulla scheda (la cartella `node/` di questo repository, già compilata — `npm run build -w node`) e configurarlo per **avviarsi automaticamente all'accensione** (un servizio di sistema, non un comando da digitare ogni volta).
...
2. Cerca il nome rete scritto sul cartello (es. `ARALD-RIFUGIO-ALPI`) e inserisce la password indicata.
3. Se ha installato l'app ARALD, può anche **inquadrare il QR code** sul cartello invece di digitare nome e password a mano.
