...
Riferimento completo: [`SPECIFICATION.md`](./SPECIFICATION.md) §8-9, §39-40, §85-86, §103-104. Per un inventario esplicito di *quali servizi* attivare per ciascuno dei pilot sotto (mesh-native vs via Project NOMAD, con una raccomandazione di pacchetto pre-confezionato per contesto) vedi [`docs/service-catalog.md`](./service-catalog.md) — questo file resta la fonte per hardware/target/scenario, quello per il "cosa accendere".
...
| Gateway / NOMAD completo | Mini-PC, **ARALD Portable** (SSD avviabile) o **ARALD Box** (SBC dedicata, vedi sotto) | AI, cache grandi, orchestrazione Docker di Project NOMAD |
| Futuro/ricerca | Hardware radio dedicato (LoRa) | backbone a lunga distanza, fuori scope MVP (§45) |

**Nota, 5 settembre 2026** (vedi "ARALD Box e PORTABLE" sotto per il dettaglio): la riga "Futuro/ricerca" sopra descriveva LoRa come backbone a lunga distanza separato, non ancora legato a un dispositivo specifico. Con l'aggiunta di un modulo LoRa raccomandato per **ARALD Box**/**ARALD Portable** sotto, questa riga va letta insieme a quella colonna "Gateway / NOMAD completo": lo stesso hardware radio, non più solo "futuro/ricerca" isolato, ma un componente raccomandato fin da questa fase per non lasciare Box/Portable senza il mezzo di trasmissione che **ARALD Card**/**ARALD Relay** hanno già nel loro piano hardware (`docs/beacon.md`).
...
Un gateway è una funzione software, non un hardware specifico: qualunque nodo con accesso simultaneo a ARALD e a un'altra rete (tipicamente Internet). Nel primo prototipo può essere semplicemente il computer di sviluppo. Non si assume che uno smartphone sia il gateway nel modello di deployment iniziale (§39) per via di consumo batteria, gestione del background da parte del sistema operativo, NAT e instabilità del processo.
...
## L'ARALD Hub come sistema portatile (SSD avviabile) — valutato dall'utente con il proprio socio, 31 agosto 2026

**Decisione architetturale**: il ruolo "PC"/"NOMAD server" del modello a due livelli sopra non deve legarsi a un computer specifico. L'intero ambiente software di Project NOMAD (sistema operativo, Docker, i suoi container, dati, modelli AI) vive su un **SSD USB esterno avviabile**; qualunque computer compatibile che lo monta ed esegue il boot da quell'unità diventa temporaneamente l'ARALD Hub. Il computer in sé non è "l'ARALD Hub" — è solo l'host hardware che lo esegue in quel momento. Coerente con §8/§86 della specifica, che già distingue "gateway come funzione software" da un hardware dedicato: questa decisione applica lo stesso principio al ruolo "PC" del modello a due livelli, non solo al "gateway".
...
### ARALD Box e PORTABLE — due deployment target paritetici (specifica ricevuta dall'utente, 2 settembre 2026)
...
**Terminologia — da non confondere con `nomad-hub/` di questo repository**: la specifica chiama i due deployment target di Project NOMAD **ARALD Box** e **ARALD Portable**, mai "ARALD Hub". Il termine "ARALD Hub" in questo progetto indica invece `nomad-hub/`, la Management API di questo repository (`docs/security.md` voce #52) che amministra Docker su *qualunque* host esegua Project NOMAD — funziona identica sotto BOX o PORTABLE, senza bisogno di alcuna modifica: parla solo con la Docker Engine API, mai con un'assunzione su quale SBC/PC la ospiti.

**ARALD Portable** — coincide con quanto già descritto sopra ("L'ARALD Hub come sistema portatile"): stesso Netac Portable SSD 250 GB di riferimento, stesso principio "l'host è hardware sostituibile, l'ambiente NOMAD vive sull'SSD". La specifica non cambia nulla di sostanziale qui, solo il nome formale del target.

**Aggiunta, 5 settembre 2026 — modulo LoRa raccomandato anche per PORTABLE**: a differenza di BOX, PORTABLE non ha una scheda tecnica fissa propria — è "qualunque host x86-64 compatibile" che monta l'SSD. La raccomandazione qui non può quindi essere un componente sulla distinta hardware come per BOX, ma una condizione sull'host scelto: **l'host su cui gira PORTABLE dovrebbe avere anche un modulo LoRa collegato** (tipicamente un dongle USB-LoRa esterno, es. basato su SX126x/SX127x — la via più semplice su un host x86-64 generico, che quasi mai ha un header UART/SPI accessibile come una SBC) — altrimenti un deployment PORTABLE perderebbe lo stesso mezzo di trasmissione che BOX/Card/Relay avrebbero. Stesso principio "hardware abstraction" già descritto sopra per il resto di PORTABLE: il software di gateway non deve assumere *quale* host esegue l'SSD, ma la lista di requisiti/checklist per chi sceglie l'host dovrebbe includere anche questo, non solo RAM/USB3/Wi-Fi-Ethernet già elencati.

**ARALD Box** — concetto nuovo: un dispositivo dedicato singolo che esegue l'intero stack (Core/Gateway/Web UI/DB/AI/Radio) in modo permanente, senza SSD esterno né host da "prendere in prestito". Hardware di riferimento per il prototipo, fornito dall'utente:
...
| Radio LoRa (aggiunta, 5 settembre 2026) | Non integrata nella SBC di riferimento — va aggiunto un modulo LoRa esterno (chipset SX126x/SX127x), via USB (dongle USB-LoRa) o via header UART/SPI/GPIO dell'Orange Pi 4 Pro (un breakout tipo Ra-02/RFM95 collegato direttamente alla scheda, opzione più stabile per un dispositivo fisso rispetto a un dongle USB) |
| USB | almeno una porta USB 3.0 (una porta va riservata al modulo LoRa se collegato via USB) |
...
**Perché aggiungere LoRa, richiesta esplicita dell'utente (5 settembre 2026)**: la tabella originale del prototipo (Wi-Fi 6/Bluetooth 5.4/Ethernet) non includeva alcuna radio a lungo raggio — un gap reale rispetto alla **ARALD Card**, che secondo `docs/beacon.md` include BLE **e** LoRa fin dal proprio piano hardware (via firmware, non SKU separati). Senza un modulo LoRa, un ARALD Box perderebbe uno dei mezzi di trasmissione della mesh rispetto a una Card/Relay nelle vicinanze — un limite tanto più rilevante quanto più **BOX è pensato come l'elemento principale/permanente** di un deployment (parole dell'utente), non un dispositivo opportunistico come uno smartphone: è la sede più naturale per un link LoRa a lungo raggio verso Fixed Relay/Card distanti, e la sua assenza sarebbe il gap più costoso dell'intero ecosistema. Per questo la raccomandazione LoRa è segnata qui come prioritaria per BOX rispetto a PORTABLE (sopra) — pur restando raccomandata per entrambi.

**Tre avvertenze esplicite prima di prendere questo hardware come raccomandazione, non solo come riferimento del prototipo**:

1. **Il chip "Allwinner A733" non è stato verificato in modo indipendente in questa sessione** — nessun accesso a internet reale per confermarne la scheda tecnica esatta (stessa limitazione già segnalata per ogni altro dato hardware in questi file). Documentato come fornito, non come fatto verificato. Lo stesso vale per il modulo LoRa aggiunto sopra: nessun modello/part number specifico raccomandato in modo verificato, solo la famiglia di chipset (SX126x/SX127x) già citata altrove in questo repository per l'ARALD Card/Relay.
...
3. **Verifica richiesta dall'utente: nessun dispositivo di questo progetto ha un driver software LoRa verificato contro hardware reale, BOX/PORTABLE inclusi una volta dotati del modulo sopra** — controllato in `node/src/transports/lora.ts` (`LoraSimulatedTransport`/`LoraMedium`, `docs/security.md` voce #51): è un transport **interamente simulato** (nessun accesso seriale/SPI a un chip SX126x/SX127x reale, MTU/latenza sono numeri scelti per modellare la radio, non misurati su hardware). Questo è un limite **universale**, non specifico di BOX/PORTABLE: la stessa cosa vale oggi per l'ARALD Card e l'ARALD Relay (`docs/beacon.md`), che condividono lo stesso codice simulato. Il fatto architetturalmente rilevante, e la ragione per cui questo non è un vicolo cieco: l'interfaccia `Transport` (`node/src/transport.ts`) astrae già la logica di trasporto dal resto della mesh (routing, store-and-forward, priorità, cifratura) — un driver LoRa reale per un chip SX126x/SX127x potrebbe sostituire `LoraSimulatedTransport` dietro la stessa interfaccia senza toccare `NomadNode`/`routing.ts`/nessun altro modulo core, esattamente come già oggi BLE/LoRa simulati coesistono con TCP reale sullo stesso nodo (`tests/integration/mixed-transport.test.ts`). **Un primo driver reale è stato effettivamente scritto lo stesso giorno**: `node/src/transports/lora-serial.ts` (`LoraSerialTransport`, `docs/security.md` voce #61) parla con un chip SX127x tramite un bridge seriale (motivato proprio dalla raccomandazione sopra — un modulo/dongle USB, l'unico percorso di collegamento portabile sia su BOX sia su PORTABLE) e ne implementa la logica di registro/FIFO fedelmente al datasheet pubblico, validata contro un fake device — ma **mai contro un chip vero**, nessun hardware disponibile in questa sessione, e nessun wiring su `cli.ts` (resta un candidato futuro). Il gap pratico "gestire LoRa come canale di comunicazione con hardware reale oggi" resta quindi aperto per tutta la famiglia, ma non è più un vicolo cieco solo a livello architetturale — la maggior parte del lavoro di driver stesso (framing byte-level, comandi di registro) è ora scritta, non solo ipotizzata; resta la verifica contro un chip vero e la gestione di duty-cycle/regolamentazione EU868 già segnalata in `docs/beacon.md`.

**Aggiunta, 9 settembre 2026 — ROCK 3C (Radxa) come variante low-cost/periferica di ARALD Box**: valutazione ricevuta dall'utente, non una sostituzione dell'Orange Pi 4 Pro sopra — un secondo profilo hardware per lo stesso ruolo "Box":

| Componente | Orange Pi 4 Pro (riferimento sopra) | ROCK 3C |
|---|---|---|
| SoC | Allwinner A733 | Rockchip RK3566 |
| CPU | 2× Cortex-A76 + 6× Cortex-A55 @ 2 GHz | 4× Cortex-A55 @ 1,6 GHz |
| RAM | 4 GB LPDDR5 | 4 GB LPDDR4 |
| NPU | 3 TOPS | assente |
| Wi-Fi / Bluetooth | Wi-Fi 6 / Bluetooth 5.4 | Wi-Fi 6 / Bluetooth 5.4 (variante più recente della scheda scelta esplicitamente dall'utente; dipende comunque dalla revisione hardware effettiva, non verificato in modo indipendente in questa sessione) |
| NVMe | PCIe 3.0 | PCIe 2.0 |
| Dimensioni | 89 × 56 mm | 85 × 56 mm |

L'assenza di NPU **non è una vera rinuncia per il carico previsto**: come già scritto nella tabella "Alternative hardware" sotto per il Rockchip RK3588, Ollama non sa usare un NPU dedicato senza un runtime specifico del produttore (RKLLM o equivalente) — un limite condiviso anche dall'NPU da 3 TOPS dell'Orange Pi 4 Pro, mai risolto in questo repository. La differenza che conta davvero per l'inferenza AI locale resta la CPU (2× Cortex-A76 contro solo Cortex-A55) — motivo per cui l'Orange Pi 4 Pro resta preferibile per il ruolo di **Box centrale** (dove gira `AiGateway`/Ollama sotto carico), mentre il ROCK 3C è candidato per **Box periferici, relay, gateway secondari e installazioni dove il costo è prioritario** — ruoli dove il carico è prevalentemente Docker/servizi/DB/Web UI/networking, non inferenza AI.

BOM prototipale indicativa (fornita dall'utente, margine di fluttuazione al rialzo ~30% da considerare su entrambe le colonne — nessuna verifica indipendente di prezzi/disponibilità in questo ambiente):

| Voce | Orange Pi 4 Pro | ROCK 3C |
|---|---|---|
| SBC | 100–120 € | 50–80 € |
| NVMe 256 GB | 20–30 € | 20–30 € |
| Alimentatore | 8–12 € | 8–12 € |
| Case | 10–15 € | 8–15 € |
| LoRa/accessori | 10–15 € | 10–15 € |
| Cavi/accessori | ~5 € | ~5 € |
| **Totale indicativo** | **~180 €** | **~125 €** |

Resta valido quanto già scritto sopra per l'Orange Pi 4 Pro (chip non verificato indipendentemente, LoRa esterno da aggiungere via USB/header, driver LoRa mai verificato contro hardware reale) — si applica identicamente al ROCK 3C, stesso trattamento "riferimento fornito, non fatto verificato".

**Relazione con gli altri modelli già documentati**: BOX **non sostituisce** il modello a due livelli (gateway + "PC") né la tabella "Alternative hardware" sotto — la riga "Gateway con NPU incorporato" di quella tabella (Orange Pi 5/Radxa Rock 5C, RK3588) resta valida come categoria, e BOX ne è ora un'istanza concreta e completamente specificata. Restano quindi **tre opzioni parallele** per il ruolo "PC"/hardware AI di un deployment, nessuna esclusiva: (a) modello a due livelli con mini-PC usato/ricondizionato, (b) ARALD Portable (SSD avviabile) su un host x86-64 già disponibile, (c) ARALD Box come dispositivo dedicato singolo. La scelta tra le tre resta condizionata dallo stesso benchmark hardware reale mai eseguito.

**Espansione correlata, documentata separatamente**: **ARALD BEACON** (`docs/beacon.md`, proposta ricevuta dall'utente il 3 settembre 2026) è un terzo ramo del progetto nello stesso spirito di BOX/PORTABLE — dispositivi fisici dedicati per un ecosistema di richiesta soccorso, ora centrati sulla **ARALD Card** (un unico hardware in formato carta di credito, configurabile via firmware in modalità Beacon/Relay/Beacon+Relay — l'utente ha successivamente scelto di unificare così quanto originariamente proposto come due dispositivi distinti, Beacon e Mobile Relay) più il **Fixed Relay** (stessa architettura, installazione permanente) — ma a differenza di BOX/PORTABLE (confezionamento di Project NOMAD, un progetto esterno) si appoggiano in buona parte a meccanismi già presenti in *questo* repository (priorità EMERGENCY, firma Ed25519, store-and-forward, rate limiting, transport BLE/LoRa simulati). Tenuti in un documento a parte perché descrivono dispositivi diversi (Beacon/Relay/Emergency Node) con un proprio protocollo di messaggio, non un'altra forma di packaging del "ARALD Hub"/"PC" di questa sezione.

**Hardware Abstraction Layer / Capability Manager di Project NOMAD** (principio architetturale della specifica §8, concetto non ancora implementato in Project NOMAD — restava così finché non si aggiorna questo paragrafo): il software non deve assumere CPU/RAM/GPU/NPU/rete/USB specifici — un processo di *hardware detection* al boot produce un "NOMAD Hardware Profile" (JSON con architettura, core CPU, RAM, presenza GPU/NPU/Wi-Fi/Bluetooth/Ethernet/USB3/storage) usato per scegliere automaticamente profilo AI e servizi attivi, con degradazione controllata (`missing capability ≠ system failure`: niente Wi-Fi → solo Ethernet, niente NPU → AI su CPU/GPU, niente radio → servizio radio disattivato, RAM bassa → profilo AI ridotto, nessun componente opzionale può impedire l'avvio del Core). Concettualmente coerente con quanto già scritto sopra ("un livello concettuale 'NOMAD Host' astrae CPU/RAM/storage/rete/USB... così il Core non deve mai essere sviluppato per un modello di PC specifico") — la nuova specifica lo rende più esplicito e operativo (un JSON concreto, una matrice di test per capability), ma resta nel perimetro di Project NOMAD stesso (auto-configurazione interna di AI/servizi al boot), non di questo repository.

**Da non confondere con il Capability Manager di `nomad-hub/`** (voce #53, `docs/security.md`): stesso nome, scopo diverso e più modesto — `nomad-hub/capability-manager.ts` (`GET /api/hub/capabilities`) legge solo CPU/RAM/storage/rete della macchina che fa girare `nomad-hub` stesso (via `node:os`/`node:fs`, dati sempre reali, mai indovinati — `gpu`/`npu`/`bluetooth`/`usb3` restano sempre `null`), e li espone in sola lettura a un operatore umano tramite un endpoint HTTP. Non sceglie automaticamente alcun profilo AI/servizio (quello resta il Capability Manager di Project NOMAD sopra, tuttora non implementato) — è solo un pannello diagnostico "questo host ha abbastanza RAM?", costruito perché era l'unico pezzo del §8 genuinamente realizzabile senza un Orange Pi reale.
...
- 1 "PC"/mini-PC per i contenuti pesanti e l'AI locale (il "NOMAD server" — Docker, corrisponde alla "cucina" della spiegazione sopra; idealmente **ARALD Portable** — un SSD avviabile con l'intero ambiente NOMAD, così l'host fisico diventa intercambiabile — o **ARALD Box** come dispositivo dedicato, vedi "ARALD Box e PORTABLE" sopra per entrambe le opzioni)
