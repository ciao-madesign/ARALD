...
- ~~Rigenerare `mobile/android/` via `npx cap sync` per propagare `appId`/`appName` nei file di progetto Android generati (`build.gradle`, `AndroidManifest.xml`, `strings.xml`) — non eseguibile in questo ambiente (build Android nativa nota come non compilabile qui).~~ — **corretto, in parte, il 6 settembre 2026**: la premessa era imprecisa. `npx cap sync android` **funziona regolarmente** in questo ambiente (nessun blocco di rete, riverificato eseguendolo davvero) — il blocco documentato riguarda solo la *compilazione* Gradle vera (`./gradlew`), non `cap sync`. Ma `cap sync` **non riscrive comunque** `applicationId`/`namespace`/`strings.xml`/il pacchetto Java — quelli sono scritti una sola volta da `npx cap add android` e il progetto `android/` era stato generato quando il repository si chiamava ancora "Nomad-Net" (`net.nomadnet.mobile`), mai riallineato al rename. Corretto manualmente (non da `cap sync`, che non lo fa): `applicationId`/`namespace` → `io.arald.mobile` coerente con `mobile/capacitor.config.json`, `app_name`/`title_activity_main` → "ARALD", pacchetto Java spostato in `io/arald/mobile/`. Riverificato con `./gradlew help` che il blocco di rete reale sulla compilazione resta invariato (`dl.google.com` → 403 per l'Android Gradle Plugin) — vedi `mobile/README.md`, sezione "Stato della build nativa Android", per il dettaglio completo.
- Aggiornare la descrizione del repository su GitHub (non solo il nome, già fatto dall'utente) — **verificato il 6 settembre 2026**: gli strumenti GitHub MCP sono ora connessi in questa sessione (a differenza di quanto annotato qui in precedenza), ma tra quelli esposti non ce n'è uno per modificare i metadati del repository (descrizione/topics) — solo file, issue, pull request, branch, Actions, release. Resta quindi da fare a mano dall'utente (Impostazioni del repository su GitHub), non per mancanza di accesso ma per mancanza dello strumento specifico in questo server MCP.
- Nessuna ricerca formale EUIPO/WIPO/USPTO eseguita (punto 12) — resta da fare prima di un deposito di marchio reale. **Eseguita il 6 settembre 2026 una ricerca preliminare informale** (punto 21) con gli strumenti disponibili in questa sessione — nessun segnale d'allarme trovato, ma dichiarato esplicitamente come non sostitutiva della ricerca formale, che resta aperta.
- ~~Verifica visiva reale (Playwright o dispositivo fisico) del nuovo wordmark "ARALD·NETWORK" nell'app mobile~~ — **fatta il 6 settembre 2026**: eseguita con un nodo ARALD reale + `local-portal/` + Playwright (non solo lettura statica del CSS). Ha trovato un bug reale non individuato dalla verifica precedente — quella aveva controllato solo l'assenza di un vincolo di larghezza *fisso* sulla regola `.wordmark`, non il rendering effettivo: a 320px di larghezza (iPhone SE-class, ancora una larghezza reale comune) il `font-size: 1.9em` fisso della regola faceva davvero traboccare/tagliare "ARALD·NETWORK" (3 caratteri più lungo di "NOMAD·NET", per cui quella dimensione era stata originariamente tarata). Corretto in `mobile/www/styles.css` (`#home-hero .wordmark`) con `font-size: clamp(1.5em, 7.5vw, 1.9em)` — verificato con uno sweep di 7 larghezze (280-1024px): nessun overflow a nessuna larghezza, e la dimensione torna esattamente a 1.9em (30.4px) da ~400px in su, quindi l'aspetto sugli schermi grandi è invariato. L'`<h1>` separato della schermata di setup (non `class="wordmark"`, stile generico `h1` più piccolo) non ha mai mostrato overflow in nessuna larghezza testata, nessuna modifica necessaria lì.

## 18. Raccomandazioni prima della pubblicazione

- ~~Eseguire la verifica Playwright del wordmark rinominato (punto 17) prima di considerare l'app mobile pronta per una demo/pubblicazione~~ — **fatta**, vedi punto 17.
- ~~Rigenerare `mobile/android/` con `cap sync` non appena disponibile un ambiente con accesso reale a `dl.google.com`/Android Gradle Plugin.~~ — **parzialmente fatta il 6 settembre 2026**, vedi punto 17: gli identificatori di progetto (`applicationId`/`app_name`/pacchetto Java) sono stati corretti a mano. Resta da fare solo la compilazione vera (`./gradlew assembleDebug`), che richiede un ambiente con accesso reale a `dl.google.com` — non disponibile qui.
- ~~Rileggere una volta il `README.md` come farebbe un lettore esterno (nessun contesto pregresso su questo progetto) — è stato riscritto per intero in questa voce, mai rivisto da occhi diversi da chi lo ha scritto.~~ — **fatta il 6 settembre 2026**: la sezione "Project status" si era fermata a una fase molto più indietro nella storia del progetto (dashboard stile motore di ricerca, voce #27 circa) e non menzionava più nulla dell'ecosistema ARALD Card/Beacon/Relay (#54-61), della messaggistica/posizione ("Option J"), del content typing/Directed Delivery, o dell'Emergency Portal — nonostante tutti questi fossero già descritti altrove nel repository (`CLAUDE.md`, `docs/security.md`). L'albero ASCII di "Repository structure" ometteva anche `nomad-hub/`, `arald-backend/`, `local-portal/`, `mirror-portal/`, pur essendo directory reali con codice reale. Corretto con un nuovo paragrafo di riepilogo in "Project status" e quattro righe aggiunte all'albero della struttura.
...
## 21. Ricerca preliminare informale, 6 settembre 2026 — non sostituisce il punto 20

Su richiesta esplicita dell'utente ("vai con il secondo punto, lavoro legale"), eseguita una ricerca preliminare informale con gli strumenti disponibili in questa sessione (ricerca web generica + verifica disponibilità domini). **Dichiarato esplicitamente**: questa *non* è la ricerca di trademark clearance formale richiesta al punto 20 — quella resta lavoro per un professionista/avvocato dei marchi con accesso diretto a EUIPO eSearch plus, WIPO Global Brand Database e USPTO Trademark Search, tutte interfacce applicative (SPA) che gli strumenti di questa sessione non possono interrogare in modo affidabile (tentato: nessuna delle tre restituisce risultati via fetch automatico, solo pagine di presentazione del servizio). Quanto segue è solo un primo controllo di plausibilità, con margine reale di falsi negativi.

**Trovato**:
- Nessun marchio registrato "ARALD" (parola singola) risulta nei database aggregatori pubblici indicizzati dal motore di ricerca usato (es. Justia Trademarks, che copre i depositi USPTO) in nessuna classe merceologica software/rete/telecomunicazioni.
- **Araldite** (Huntsman Advanced Materials, ex Ciba-Geigy) resta l'unico marchio realmente affermato con prefisso letterale condiviso "ARAL-/ARALD-" — adesivi/resine epossidiche, classe merceologica del tutto diversa da software/rete. Confermato quanto già annotato al punto 20: un rischio di confusione diretta è improbabile per settore, ma la somiglianza fonetica/testuale resta un punto che un professionista dovrebbe valutare esplicitamente prima di un deposito reale, specie in giurisdizioni (come l'UE) dove la valutazione di confondibilità considera anche la notorietà del marchio anteriore a prescindere dalla classe.
- Due società britanniche dissolte, "ARALD Capital Limited" e "ARALD Technical Limited" — nessuna relazione con software/rete apparente, e dissolte quindi verosimilmente non più titolari di diritti attivi, ma segnalate per completezza.
- "Arald" esiste anche come nome proprio di persona/personaggio (narrativa fantasy, un modello di abito da sposa Rosa Clará) — nessuna sovrapposizione di settore.
- **Dominio**: `arald.com` risulta già registrato (una pagina di login, proprietario non identificabile dalla ricerca, nessun collegamento apparente a software/rete/mesh); `arald.io`/`arald.net`/`arald.app`/`arald.dev` e numerosi altri TLD risultavano ancora disponibili al momento della verifica (6 settembre 2026) — stessa avvertenza già al punto 19: nessuna garanzia che lo restino, e questa disponibilità non prova né smentisce nulla sul piano dei marchi (i due registri sono indipendenti).

**Non trovato/non verificabile con questi strumenti**: nessuna conferma né esclusione affidabile di un deposito attivo su EUIPO/WIPO/USPTO — l'assenza dai risultati di un motore di ricerca generico non equivale all'assenza da un registro ufficiale (deposito non ancora pubblicato, terminologia di classificazione diversa, marchi meno indicizzati dal web pubblico). **Conclusione**: nessun segnale d'allarme trovato in questa ricerca informale, ma non cambia la raccomandazione del punto 20 — resta necessaria una ricerca di clearance professionale prima di un deposito di marchio reale o di un lancio commerciale, specialmente per la posizione di Araldite/Huntsman.
