...
| CERT-002 | Certificazione modulare di XIAO ESP32-S3/Wio-SX1262 non verificata — stesso tipo di rischio di CERT-001, produttore diverso. **Aggiornamento 13 settembre 2026**: il chip SX1262 di questo kit è stato unificato come standard LoRa su BOX/PORTABLE/Card (`docs/deployment.md`, "LoRa unificato su SX1262") — il rischio si applica quindi ora all'intero ecosistema, non solo alla piattaforma di sviluppo della Card | Bassa (da confermare) | Medio (potrebbe non ridurre l'iter di conformità come sperato); impatto ora più ampio per via dell'uso su tutte le piattaforme | Decisione esplicita dell'utente: non affrontare ora — solo segnalare il punto, reperire la documentazione del produttore quando possibile, valutare a fine progettazione insieme a CERT-001 | Aperto — rimandato a fine progettazione |
