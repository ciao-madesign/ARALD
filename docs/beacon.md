...
**Stato**: Fixed/Mobile Relay restano documentazione di riferimento/pianificazione (nessun cambio di protocollo necessario, vedi sotto). **Il Relay Registry è invece implementato** (`node/src/relay-registry.ts`, pezzo 1 di 3 del piano "Relay Registry → profilo corriere Mobile/Fixed Relay → Beacon SOS" approvato dall'utente il 3 settembre 2026 — vedi `docs/security.md` voce #54 per il dettaglio completo: modulo, wiring in `NomadNode`, endpoint `GET`/`POST /api/relays` in `web-ui.ts`, `--expose-relay-registry` in `cli.ts`, pannello "Relay" + layer di pin dedicato su `mapview.js` lato mobile). Terza specifica ricevuta dall'utente lo stesso giorno (3 settembre 2026), a seguito di una discussione esplicita in cui l'utente ha **rifiutato** una precedente proposta di rebranding (una famiglia hardware unificata "NOMAD Radio Device"): Beacon, Mobile Relay e Fixed Relay restano tre nomi distinti in questo documento, non varianti di un unico prodotto. Resta aperta e non decisa, separatamente da questa sezione, l'idea di "SOS Proximity Detection" (misura RSSI lato relay per una stima di prossimità) discussa nella stessa conversazione ma non ancora integrata in documentazione.
...
2. **La correlazione "quale relay ha ricevuto questo SOS" resta da fare**: richiede che il pacchetto di emergenza porti traccia di quale relay lo ha inoltrato per primo/ricevuto — un dettaglio da definire insieme al formato messaggio del Beacon (pezzo 3), non affrontato in questo pezzo.
