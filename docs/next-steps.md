...
5. ✅ **Fatto** (`docs/security.md` voce #39, con un follow-up mirato in voce #40): `service://emergency-news` — `NewsGateway` guadagna un classificatore `isEmergencyHeadline` (default: parola chiave nel campo `category`, sovrascrivibile) che marca una headline nuova/cambiata per la pubblicazione con `{ announce: true, priority: Priority.EMERGENCY }` invece del default, così "salta la coda" via `CONTENT_ANNOUNCE` verso i peer già connessi. Un tetto (`MAX_EMERGENCY_ANNOUNCES_PER_SYNC`) limita quanti annunci proattivi un singolo sync può originare, a protezione da un feed ostile che tagga troppi item come emergenza — il rate limiting per-peer esistente non copre i pacchetti che un nodo origina da sé. La voce #40 ha poi disaccoppiato la decisione di annuncio da `publishedById` (condiviso con ogni headline di routine) con una mappa dedicata più piccola, chiudendo un caso di ri-annuncio spurio raggiungibile anche senza riavvio del processo — vedi quella voce per il dettaglio.
...
Con l'Opzione I (news evoluto) **completata per intero** e l'Opzione J (messaggistica + tracciamento posizione) valutata, e con il debito di design UI mobile, `ContentStore`, l'intera evoluzione news (parser RSS, livelli headline/summary, digest AI, `service://emergency-news`), il prerequisito architetturale condiviso e la chat 1:1 **già completati** (voci #28-40, `docs/security.md` — vedi "Stato del progetto" in `CLAUDE.md`), questo piano riordina i candidati **rimasti** in un'unica sequenza motivata — per dipendenze reali, non solo per data di scoperta — pensata per essere seguita passo per passo, non per essere fatta tutta insieme. Le fasi "0" - "4" sono barrate sotto come riferimento storico di come questo piano è stato pensato, non perché vadano ancora fatte.
...
- ~~Canali pubblici di chat (Opzione J).~~ **Ancora da fare** — unico pezzo rimasto di questa fase.
- ~~`service://emergency-news` (Opzione I)~~ ✅ Fatto (voce #39) — "Nomad News evoluto" è ora completo per intero.

**Fase 5 (ex Fase 4, unico pezzo rimasto) — Canali pubblici di chat.** Costruita sopra `CONTENT_ANNOUNCE` (#34), stesso impianto backend/UI della chat 1:1 (#36) già in produzione. Prossimo pezzo indipendente disponibile.
