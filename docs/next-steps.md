...
### Debito di design della UI mobile — da affrontare prima di aggiungere altre feature

**Segnalazione esplicita per chi riprende questo lavoro**: `mobile/www/` è stata verificata *funzionalmente* end-to-end (pairing, dashboard, chiamata servizi, ricerca, scansione QR, la nuova home "motore di ricerca" con collegamenti rapidi — voce #27) ma **non ha mai ricevuto una vera passata di design**. Ogni iterazione finora ha aggiunto/spostato funzionalità dentro lo stesso impianto visivo minimale nato per dimostrare che il flusso funzionava, non per essere l'interfaccia che un utente reale userebbe durante un'emergenza o in un rifugio. Concretamente, oggi mancano: una gerarchia visiva reale oltre a liste di card generiche; transizioni/animazioni tra stati (nulla si muove, ogni aggiornamento è un repaint istantaneo); stati di caricamento (nessuno skeleton/spinner — un campo vuoto e uno non ancora caricato sono indistinguibili); un sistema di icone coerente (oggi solo emoji, `SERVICE_ICONS` in `app.js`); gestione dell'errore oltre a un paragrafo di testo rosso; conferme/feedback tattili sulle azioni (tap su un pulsante non ha alcun riscontro oltre l'effetto finale); un vero layout responsive per tablet (pensato solo per viewport da telefono, ~390px); accessibilità oltre alle label HTML di base (nessun controllo con screen reader reale). Prima di aggiungere ulteriori funzionalità mobile (es. l'esposizione di `service://news`/`service://emergency-news` dell'Opzione I sopra), vale la pena una sessione dedicata **solo** a rivedere il design della UI — probabilmente con l'aiuto di uno strumento di design (Figma) o di un design system coerente, non altre iterazioni incrementali sullo stesso CSS scritto a mano finora.
...
1. Parser RSS/Atom scritto da zero (coerente con "niente dipendenze esterne non necessarie" — stesso precedente dell'encoder QR, `node/src/qrcode.ts`), a sostituire l'attuale mini-API JSON ad-hoc di `NewsGateway`; testabile contro un server HTTP locale di test (mai un fake server spedito, per la stessa scelta esplicita dell'utente già seguita per `NewsGateway`).
2. Schema più ricco della notizia (`source`, `category`, `language`, `updated_at`) — estensione diretta del modello dati (`NewsHeadline`, `news-gateway.ts`) e del pattern di firma esistente.
...
**Lavoro architetturale genuinamente nuovo, non solo estensione** (il pezzo più interessante emerso dall'analisi): oggi la priorità di un pacchetto è fissa **per tipo di messaggio**, non parametrizzabile per singolo contenuto — ogni `CONTENT_*` usa sempre `Priority.CONTENT`. La scoperta dei contenuti è inoltre **pull-based** (`CONTENT_QUERY`/`CONTENT_FOUND`), senza un annuncio broadcast come `SERVICE_ANNOUNCE` per i servizi. "Un bollettino P0 salta la coda e raggiunge subito tutti i nodi" (come descritto nella proposta) non è quindi gratis: serve o un nuovo meccanismo di annuncio broadcast per contenuti ad alta priorità (parallelo a `SERVICE_ANNOUNCE`, stessa disciplina di firma), oppure una priorità dichiarata propagata attraverso `SYNC_RESPONSE`. Vedi anche `docs/protocol.md`, sezione "Priorità (§50)". Questo è il lavoro più sostanzioso se si vuole realizzare davvero l'idea di emergenza descritta, non solo un tag cosmetico di categoria.
...
**Se si riprende, ordine di implementazione consigliato**: (1) parser RSS + schema arricchito, (2) livelli headline/summary, (3) digest AI componendo `NewsGateway`+`AiGateway`, (4) l'estensione di priorità/broadcast per contenuti (il pezzo architetturale nuovo) → `service://emergency-news` come conseguenza naturale. Stesso workflow a doppio check di ogni voce precedente (implementa, testa, `code-review`, correggi, documenta, commit+push su entrambi i branch).
