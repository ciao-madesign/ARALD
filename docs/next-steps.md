...
## Piano d'insieme (25 agosto 2026, aggiornato dopo #28-39) — come ordinare tutti i candidati aperti

Con l'Opzione I (news evoluto) **completata per intero** e l'Opzione J (messaggistica + tracciamento posizione) valutata, e con il debito di design UI mobile, `ContentStore`, l'intera evoluzione news (parser RSS, livelli headline/summary, digest AI, `service://emergency-news`), il prerequisito architetturale condiviso e la chat 1:1 **già completati** (voci #28-39, `docs/security.md` — vedi "Stato del progetto" in `CLAUDE.md`), questo piano riordina i candidati **rimasti** in un'unica sequenza motivata — per dipendenze reali, non solo per data di scoperta — pensata per essere seguita passo per passo, non per essere fatta tutta insieme. Le fasi "0" - "4" sono barrate sotto come riferimento storico di come questo piano è stato pensato, non perché vadano ancora fatte.
