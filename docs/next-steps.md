...
Con l'Opzione I (news evoluto) **completata per intero**, la messaggistica dell'Opzione J (chat 1:1 + canali pubblici) **completata per intero**, l'Opzione K (`service://internet-fetch`, "Internet senza Internet") **completata per intero**, e il tracciamento posizione dell'Opzione J ancora da valutare/decidere con l'utente — e con il debito di design UI mobile, `ContentStore`, l'intera evoluzione news (parser RSS, livelli headline/summary, digest AI, `service://emergency-news` con il suo follow-up #41), il prerequisito architetturale condiviso, la chat 1:1 e i canali pubblici **già completati** (voci #28-42, `docs/security.md` — vedi "Stato del progetto" in `CLAUDE.md`), questo piano riordina i candidati **rimasti** in un'unica sequenza motivata — per dipendenze reali, non solo per data di scoperta — pensata per essere seguita passo per passo, non per essere fatta tutta insieme. Le fasi "0" - "5" sono barrate sotto come riferimento storico di come questo piano è stato pensato, non perché vadano ancora fatte.
...
~~**Fase 3 — Feature veloci e in gran parte indipendenti tra loro (parallelizzabili).**~~ ✅ Fatta per intero.
...
~~**Fase 4 — Costruita sopra il prerequisito della Fase 2.**~~ ✅ Fatta per intero.
...
~~**Fase 5 (ex Fase 4, unico pezzo rimasto) — Canali pubblici di chat.**~~ ✅ Fatta (voce #40) — con questa, la messaggistica dell'Opzione J è completa a eccezione delle chat private/di gruppo cifrate (Fase 7 sotto).

**Fase 6 — Richiede prima una decisione esplicita con l'utente, non solo codice.** Tracciamento posizione (Opzione J): consenso, conservazione, autorizzazione all'interrogazione del registro vanno decisi **con** l'utente prima di scrivere il gateway registro o il lato mobile — la specifica stessa (§56) elenca la posizione tra i dati da minimizzare, non è una scelta tecnica da prendere in autonomia. **Prossimo pezzo indipendente disponibile**, ma richiede questa conversazione preliminare prima di scrivere codice.

**Fase 7 — Il pezzo di design crittografico più sostanzioso, per ultimo.** Chat private/di gruppo cifrate (Opzione J): key-wrapping, gestione ingresso/uscita dal gruppo — conviene affrontarlo dopo aver validato chat 1:1 e canali pubblici (entrambi ora in produzione e testati), quando il resto dell'infrastruttura di messaggistica è già solido.
