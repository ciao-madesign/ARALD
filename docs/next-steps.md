...
5. ✅ **Fatto** (`docs/security.md` voce #39): `service://emergency-news` — `NewsGateway` guadagna un classificatore `isEmergencyHeadline` (default: parola chiave nel campo `category`, sovrascrivibile) che marca una headline nuova/cambiata per la pubblicazione con `{ announce: true, priority: Priority.EMERGENCY }` invece del default, così "salta la coda" via `CONTENT_ANNOUNCE` verso i peer già connessi. Un tetto (`MAX_EMERGENCY_ANNOUNCES_PER_SYNC`) limita quanti annunci proattivi un singolo sync può originare, a protezione da un feed ostile che tagga troppi item come emergenza — il rate limiting per-peer esistente non copre i pacchetti che un nodo origina da sé.
...
## Piano d'insieme (25 agosto 2026, aggiornato dopo #28-40) — come ordinare tutti i candidati aperti

Con l'Opzione I (news evoluto) **completata per intero**, la messaggistica dell'Opzione J (chat 1:1 + canali pubblici) **completata per intero**, e il tracciamento posizione dell'Opzione J ancora da valutare/decidere con l'utente — e con il debito di design UI mobile, `ContentStore`, l'intera evoluzione news (parser RSS, livelli headline/summary, digest AI, `service://emergency-news`), il prerequisito architetturale condiviso, la chat 1:1 e i canali pubblici **già completati** (voci #28-40, `docs/security.md` — vedi "Stato del progetto" in `CLAUDE.md`), questo piano riordina i candidati **rimasti** in un'unica sequenza motivata — per dipendenze reali, non solo per data di scoperta — pensata per essere seguita passo per passo, non per essere fatta tutta insieme. Le fasi "0" - "5" sono barrate sotto come riferimento storico di come questo piano è stato pensato, non perché vadano ancora fatte.
