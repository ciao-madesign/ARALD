 ...
 * `--nomad-url` points at a real Project NOMAD/Kiwix instance instead. Not
 * used by the automated test suite (tests/integration/nomad-gateway.test.ts
 * builds its own fixtures directly), same relationship
 * `tools/simulator/cli.ts` has to its own `simulate.ts`.
 */
async function main(): Promise<void> {
  ...
  const gateway = new KiwixGateway(node, nomadBaseUrl);
  const published = await gateway.syncCatalog();
  gateway.registerSearchService();
