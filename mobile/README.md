...
- `www/` — l'app vera e propria: `index.html` + `styles.css` + `app.js`, vanilla JS senza framework (stessa disciplina di `node/src/web-ui.ts`: ogni valore che arriva dalla rete passa da `textContent`, mai `innerHTML`, perché il nome di un contenuto o l'id di un servizio dichiarati da un peer sono input non fidato). Due schermate:
  ...
  - **Dashboard**: apertura in stile "motore di ricerca" (su richiesta esplicita dell'utente, `docs/security.md` voce #27) — wordmark NOMAD-NET e casella di ricerca contenuti a pillola, seguiti immediatamente dal pannello **Servizi**: una card per servizio disponibile (icona SVG, nome leggibile — "Assistente AI", "Enciclopedia", "Notizie" — id tecnico come sottotitolo, tag delle capacità, bottone "Chiama" a piena larghezza), il primo pannello che l'utente vede, prima ancora delle statistiche del nodo — su richiesta esplicita dell'utente ("un utente comune deve vedere subito cosa può fare"), al posto della precedente riga di piccoli collegamenti rapidi rimossa (`docs/security.md` voce #29). Sotto: stato del nodo (vicini, servizi, cache, relay), lista vicini con trust level, contenuti sfogliabili/cercabili. Toccare "Chiama" apre il form dedicato (un campo prompt per `service://ai`, JSON generico per gli altri, incluso `service://news`) espandendo quella card a piena larghezza. Auto-refresh ogni 5s, come la dashboard desktop. Passata di design completa (icone SVG coerenti, skeleton di caricamento, transizioni, feedback tattile, layout tablet, accessibilità) in `docs/security.md` voce #28.
