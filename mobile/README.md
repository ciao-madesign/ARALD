...
- `www/` — l'app vera e propria: `index.html` + `styles.css` + `app.js`, vanilla JS senza framework (stessa disciplina di `node/src/web-ui.ts`: ogni valore che arriva dalla rete passa da `textContent`, mai `innerHTML`, perché il nome di un contenuto o l'id di un servizio dichiarati da un peer sono input non fidato). Due schermate:
  - **Setup/pairing**: inserisci indirizzo del gateway (`host:porta`) e token di pairing (stampati in console dal gateway quando avviato con `--web-host`/`--allow-service-calls`, vedi `docs/development.md`). Verifica la raggiungibilità con una `GET /api/status` prima di salvare, mostra un errore chiaro altrimenti.
...
Poi apri `http://127.0.0.1:8090`, inserisci `127.0.0.1:8080` e il token stampato dal terminale 1.
...
**Nota sulla persistenza del token**: `app.js` salva indirizzo del gateway e token di pairing in `localStorage`, in chiaro — scelta deliberata per questo primo prototipo (nessun meccanismo di storage cifrato aggiunto), non un oversight. Su un dispositivo condiviso o compromesso, chiunque abbia accesso allo storage della WebView (es. `adb backup`/`adb shell run-as` su un dispositivo debuggabile o rootato) può recuperare un token valido e chiamare `POST /api/call` per conto dell'app. Accettabile per un prototipo di validazione; da rivedere (es. `@capacitor/preferences` con storage cifrato via Android Keystore, o un token con scadenza) prima di un uso reale su un dispositivo non fidato.
